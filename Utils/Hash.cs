...
namespace EvolutionSimulator.Utils
{
	public static class Hash
	{
		...
		public static int GetHashCodeForATKV(int[] array, int someInt)
		{
			byte[] a = new byte[4 * (array.Length + 1)];
			int index = 0;
			for (int i = 0; i < array.Length; i++)
			{
				for (int j = 0; j < 4; j++)
		...
		public static byte[] GetShuffledByteArray()
		{
			byte[] res = new byte[256];
			for (int i = 0; i <= 255; i++) res[i] = (byte)i;

			Random r = new Random();
			int iters = r.Next(256, 256 * 10 + 1);
			int swapIndex1, swapIndex2;
			byte temp;

			for (int i = 0; i < iters; i++)
			{
				swapIndex1 = r.Next(0, 256);
				swapIndex2 = r.Next(0, 256);
				if (swapIndex1 == swapIndex2) continue;
				temp = res[swapIndex1];
				res[swapIndex1] = res[swapIndex2];
				res[swapIndex2] = temp;
			}
			return res;
		}
